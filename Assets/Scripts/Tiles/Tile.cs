...
public abstract class Tile : MonoBehaviour
{
    ...
    void OnMouseDown() {
        ...
        {
            // Show upgrade panel if we clicked a tile with a turret
            if (OccupiedUnit != null && OccupiedUnit is BaseTurret)
            {
                TurretUpgradeUI upgradeUI = FindFirstObjectByType<TurretUpgradeUI>();
                if (upgradeUI != null)
                {
                    upgradeUI.ShowUpgradePanel(OccupiedUnit as BaseTurret);
                }
            }
        }
